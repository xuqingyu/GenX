...
function fleccs4(EP::Model, inputs::Dict,  FLECCS::Int, UCommit::Int, Reserves::Int)
	...
	@expression(EP, ePower_use_heater[y in FLECCS_ALL,t=1:T], FLECCS_parameters[!,:pPowerUseRate_heater][y] * vSTEAM_in[y,t])
	...
	#@constraint(EP, cMinGeneration_gt[y in FLECCS_ALL,t=1:T], vP_gt[y,t] >= dfGen_ccs[(dfGen_ccs[!,:R_ID].==y),:Min_power][NGCT_id] *EP[:eTotalCapFLECCS][y,NGCT_id])
    # Min power generated for steam TURBINE at hour "t"
    #@constraint(EP, cMinGeneration_st[y in FLECCS_ALL,t=1:T], ePower_st[y,t] >=   dfGen_ccs[(dfGen_ccs[!,:R_ID].==y),:Min_power][NGST_id]* EP[:eTotalCapFLECCS][y,NGST_id])
    # Min Power used for compressing CO2 
    #@constraint(EP, cMin_comp[y in FLECCS_ALL,t=1:T], ePower_use_comp[y,t] >=   dfGen_ccs[(dfGen_ccs[!,:R_ID].==y),:Min_power][Comp_id]  *EP[:eTotalCapFLECCS][y,Comp_id])
    
	# Min captured CO2  from adsorber at time t should be less than the capacity of capture unit [tonne CO2]
	#@constraint(EP, cMinCapture[y in FLECCS_ALL,t=1:T], vCAPTURE[y,t] >=  dfGen_ccs[(dfGen_ccs[!,:R_ID].==y),:Min_power][PCC_id] * EP[:eTotalCapFLECCS][y,PCC_id] )
	...
	@constraints(EP, begin
	    [y in FLECCS_ALL, i in NGCT_id, t = 1:T],vFLECCS_output[y,i,t] == vP_gt[y,t]
		[y in FLECCS_ALL, i in NGST_id,t = 1:T],vFLECCS_output[y,i,t] == ePower_st[y,t]	
		[y in FLECCS_ALL, i in PCC_id,t = 1:T],vFLECCS_output[y,i,t] == vCAPTURE[y,t]
		[y in FLECCS_ALL, i in Comp_id, t =1:T],vFLECCS_output[y,i,t] == ePower_use_comp[y,t]	
		[y in FLECCS_ALL, i in HeatPump_id,t = 1:T],vFLECCS_output[y,i,t] == ePower_use_ts[y,t]
		[y in FLECCS_ALL, i in Hot_id, t =1:T],vFLECCS_output[y,i,t] == vSTORE_hot[y,t]
		[y in FLECCS_ALL, i in Cold_id, t =1:T],vFLECCS_output[y,i,t] == vSTORE_cold[y,t]	
		[y in FLECCS_ALL, i in Heater_id, t =1:T],vFLECCS_output[y,i,t] == ePower_use_heater[y,t]
