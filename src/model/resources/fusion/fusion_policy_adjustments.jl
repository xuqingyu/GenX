...
#################################
#
#    Capacity Reserve Margin peakload
#
#################################
...
function fusion_capacity_reserve_margin_peakload_adjustment!(EP::Model,
    ...
    for y in applicable_resources
        _fusion_capacity_reserve_margin_peakload_adjustment!(EP, inputs, resource_component(y), y)
    end
end


function _fusion_capacity_reserve_margin_peakload_adjustment!(EP::Model,
        inputs::Dict,
        resource_component,
        y::Int)
    ...
    for capres_zone in 1:ncapres
        t_peak = peak_idx[capres_zone]
        adjustment = fusion_capacity_reserve_margin_peakload_adjustment(
            EP, inputs, resource_component, y, capres_zone, [t_peak])
        add_to_expression!(eCapResMarBalance[capres_zone, t_peak], adjustment)
    end
    return
end
...
function fusion_capacity_reserve_margin_peakload_adjustment(EP::Model,
    ...
    capacity_to_underway_adj = capresfactor * (ePulseUnderway[t_peak] .- eTotalCap)
    parasitic_adj = _fusion_crm_parasitic_adjustment(
        capresfactor, ePassive[t_peak], eActive[t_peak], eStartPower[t_peak])
    dwell_adj = -capresfactor *
                _fusion_dwell_avoided_operation(dwell_time, ePulseStart[t_peak])

    total_adj = @expression(EP, capacity_to_underway_adj + parasitic_adj + dwell_adj)

    # Cancel out the dependence on down_var, since CRM is related to power, not capacity
    if y in ids_with_maintenance(gen)
        maint_adj = thermal_maintenance_and_fusion_capacity_reserve_margin_peakload_adjustment(
...
# alias for better parallelism in effective_capacity.jl
const thermal_fusion_capacity_reserve_margin_peakload_adjustment = fusion_capacity_reserve_margin_peakload_adjustment!
