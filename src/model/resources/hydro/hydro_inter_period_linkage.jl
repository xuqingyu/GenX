...
function hydro_inter_period_linkage(EP::Model, inputs::Dict)
	...
	@variable(EP, vSOC_HYDROw[y in HYDRO_RES, n in MODELED_PERIODS_INDEX] >= 0)

	# Build up in storage inventory over each representative period w
	# Build up inventory can be positive or negative
	@variable(EP, vdSOC_HYDRO[y in HYDRO_RES, w=1:REP_PERIOD])
	...
	@constraint(EP, cSoCBalLongDurationStorageStart_H[w=1:REP_PERIOD, y in HYDRO_RES],
				    EP[:vS_HYDRO][y,hours_per_subperiod*(w-1)+1] == (EP[:vS_HYDRO][y,hours_per_subperiod*w]-vdSOC_HYDRO[y,w])-(1/dfGen[!,:Eff_Down][y]*EP[:vP][y,hours_per_subperiod*(w-1)+1])-EP[:vSPILL][y,hours_per_subperiod*(w-1)+1]+inputs["pP_Max"][y,t]*EP[:eTotalCap][y])
	...
	@constraint(EP, cSoCBalLongDurationStorageInterior_H[y in HYDRO_RES, r in MODELED_PERIODS_INDEX[1:(end-1)]],
					vSOC_HYDROw[y,r+1] == vSOC_HYDROw[y,r] + vdSOC_HYDRO[y,dfPeriodMap[!,:Rep_Period_Index][r]])

	## Last period is linked to first period
	@constraint(EP, cSoCBalLongDurationStorageEnd_H[y in HYDRO_RES, r in MODELED_PERIODS_INDEX[end]],
					vSOC_HYDROw[y,1] == vSOC_HYDROw[y,r] + vdSOC_HYDRO[y,dfPeriodMap[!,:Rep_Period_Index][r]])
	...
	@constraint(EP, cSoCBalLongDurationStorageUpper_H[y in HYDRO_RES, r in MODELED_PERIODS_INDEX],
					vSOC_HYDROw[y,r] <= dfGen[!,:Hydro_Energy_to_Power_Ratio][y]*EP[:eTotalCap][y])
