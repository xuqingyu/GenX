...
function generate_model(setup::Dict,inputs::Dict,OPTIMIZER::MOI.OptimizerWithAttributes,modeloutput = nothing)
	...
	# Infrastructure
	EP = discharge(EP, inputs, setup["PieceWiseHeatRate"],setup["EnergyShareRequirement"])
	...
	if !isempty(inputs["THERM_ALL"])
		EP = thermal(EP, inputs, setup["UCommit"], setup["Reserves"], setup["PieceWiseHeatRate"],setup["CapacityReserveMargin"])
	end

	# Model constraints, variables, expression related to fleccs
	if (setup["FLECCS"] >= 1)
		EP = fleccs(EP, inputs, setup["FLECCS"], setup["UCommit"], setup["Reserves"], setup["CapacityReserveMargin"])
	end
