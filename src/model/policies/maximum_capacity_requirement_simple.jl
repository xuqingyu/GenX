@doc raw"""
	maximum_capacity_requirement_simple!(EP::Model, inputs::Dict, setup::Dict)
...
"""
function maximum_capacity_requirement_simple!(EP::Model, inputs::Dict, setup::Dict)
    println("Maximum Capacity Requirement Module Simple Version")
    NumberOfMaxCapSpReqs = inputs["NumberOfSimpleMaxCapReqs"]

    # if input files are present, add maximum capacity requirement slack variables
    if haskey(inputs, "MaxCapSpPriceCap")
        @variable(EP, vMaxCapSp_slack[maxcap = 1:NumberOfMaxCapSpReqs]>=0)
        add_similar_to_expression!(EP[:eMaxCapResSp], -1.0, vMaxCapSp_slack)

        @expression(EP,
            eCMaxCapSp_slack[maxcap = 1:NumberOfMaxCapSpReqs],
            inputs["MaxCapSpPriceCap"][maxcap]*EP[:vMaxCapSp_slack][maxcap])
        @expression(EP,
            eTotalCMaxCapSpSlack,
            sum(EP[:eCMaxCapSp_slack][maxcap] for maxcap in 1:NumberOfMaxCapSpReqs))

        add_to_expression!(EP[:eObj], eTotalCMaxCapSpSlack)
    end

    @constraint(EP,
        cZoneMaxCapReqSp[maxcap = 1:NumberOfMaxCapSpReqs],
        EP[:eMaxCapResSp][maxcap]<=inputs["MaxCapReqSp"][maxcap])
end
