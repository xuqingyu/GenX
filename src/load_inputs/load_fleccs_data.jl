...
function load_fleccs_data(setup::Dict, path::AbstractString, sep::AbstractString, inputs_ccs::Dict, fuel_costs::Dict, fuel_CO2::Dict)
	...
	if setup["FLECCS"] == 1
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data1.csv"), header=true), copycols=true)
		FLECCS_parameters = DataFrame(CSV.File(string(path,sep,"FLECCS_data1_process_parameters.csv"), header=true), copycols=true)
		println("FLECCS_data1.csv Successfully Read!, NGCC-CCS without flexible subcompoents")
	elseif setup["FLECCS"] == 2
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data2.csv"), header=true), copycols=true)
	...
	elseif setup["FLECCS"] == 3
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data3.csv"), header=true), copycols=true)
	...
	elseif setup["FLECCS"] == 4
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data4.csv"), header=true), copycols=true)
	...
	elseif setup["FLECCS"] == 5
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data5.csv"), header=true), copycols=true)
		FLECCS_parameters = DataFrame(CSV.File(string(path,sep,"FLECCS_data5_process_parameters.csv"), header=true), copycols=true)
		println("FLECCS_data5.csv Successfully Read!, NGCC-CCS with hydrogen generation and storage")

	elseif setup["FLECCS"] == 6
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data6.csv"), header=true), copycols=true)
		FLECCS_parameters = DataFrame(CSV.File(string(path,sep,"FLECCS_data6_process_parameters.csv"), header=true), copycols=true)
		println("FLECCS_data6.csv Successfully Read!, NGCC-CCS with DAC")

	elseif setup["FLECCS"] == 7
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data7.csv"), header=true), copycols=true)
		FLECCS_parameters = DataFrame(CSV.File(string(path,sep,"FLECCS_data7_process_parameters.csv"), header=true), copycols=true)
		println("FLECCS_data7.csv Successfully Read!, NGCC-CCS with DAC-MIT")

	elseif setup["FLECCS"] == 8
		dfGen_ccs = DataFrame(CSV.File(string(path,sep,"FLECCS_data8.csv"), header=true), copycols=true)
		FLECCS_parameters = DataFrame(CSV.File(string(path,sep,"FLECCS_data8_process_parameters.csv"), header=true), copycols=true)
